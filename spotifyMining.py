...
def getTrackAndAlbumIDsFromPlaylist(playlistID, authToken, genre):
    ...
    for item in playlistItems['items']:
        ...
        try:
            #FYI sections jest krótsze od segments
            ...
            genre = getArtistGenres(artistID,authToken)

            listOfID.append([trackID, artistID, genre, duration, endOfFadeIn, startOfFadeOut, loudness, tempo,
...
def getArtistGenres(artistID, authToken):
    chosenArtistEndpoint = "{0}/artists/{1}".format(spotifyApi, artistID)
    getRequest = requests.get(chosenArtistEndpoint, headers=authToken)
    checkForError(getRequest)

    artistItems = getRequest.json()
    firstGenre = artistItems['genres']
    if len(firstGenre)>0:
        firstGenre = firstGenre[0]
    else:
        firstGenre = ''
